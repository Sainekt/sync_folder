        ...
        .map(|cloud_item| {
            ...
            async move {
                ...
                let Some(download_url) = response.get("href").and_then(|v| v.as_str()) else {
                    return Err(format!("[ERROR] {} | Missing href in response", cloud_item.name));
                };
                ...
                let file_response = match client.get(download_url).send().await {
                    ...
                    Err(e) => {
                        return Err(format!("[ERROR] download failed: {}, {}", cloud_item.name, e));
                    }
                };
    ...
    let upload_stream = stream
        ...
        .map(|(file_name, file_path)| {
            ...
            async move {
                ...
                let (status, response) = fetch(url, Method::GET, Some(&client)).await.map_err(|e|
                    format!("[ERROR] Fetch failed: {}", e)
                )?;
                ...
                if status != StatusCode::OK {
                    let error_msg = response
                        ...
                        .and_then(|v| v.as_str())
                        .unwrap_or("Unknown error");
                    return Err(format!("[ERROR] {} | Failed to get URL: {}", file_name, error_msg));
                }
